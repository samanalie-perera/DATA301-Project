...
The steps we can take to maintain integrity and confidentiality is making sure that only authorised users have access to the data:
To do that we can:
- Make the github repo private that way only authorised people have access (group members)
- Can password protect our data, with a password that is only distributed to the users who are authorised access
...
dim(df)
```
There are 26,627 rows and 30 features in our data. 
...
- Background and Data: Completeness of the dataset, Steps to integrate the datasets
